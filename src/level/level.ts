...
export function MUDcreateLevel(config: MUDGameConfig, levels: MUDLevel[], sizeX: number, sizeY: number): MUDLevel {
    ...
    const rooms: MUDRoom[][] = Array(sizeX).fill(0).map((_val, x) => {
        return Array(sizeY).fill(0).map((_v, y) => createShellRoom(x, y));
    });
    ...
    function getRoomByCoordinates(x: number, y: number): MUDRoom {
    ...
    function getRoomByID(id: string): MUDRoom {
        let room: MUDRoom;
...
export interface MUDLevel {
    ...
    readonly roomByCoordinates: (x: number, y: number) => MUDRoom;
    readonly roomByID: (id: string) => MUDRoom;
}
